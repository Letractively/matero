...
$vl_pass_query=mysql_query("SELECT * FROM usuario
							 WHERE id_usuario='$usuario_logueado[id_usuario]'
							 AND pass = password('$vf_passactual')
										");
if (!mysql_num_rows($vl_pass_query))
   {$vl_mensaje_error="ERROR: La password actual es incorrecta";
   $vl_error=1;
	}
