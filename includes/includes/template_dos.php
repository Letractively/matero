...
modificado por mbotta -> que la carpeta donde se encuentra la plantilla no necesita ser ./templates
sino que se decide cuando se llama la funcion set_file()
*********************************************************************************/
...
define("TEMPLATE", 1);
   ...
   $halt_on_error = "yes";   // "yes" | "report" | "no"
    ...
    function pparse($target,$archivo="", $block="", $append = false) {
        if($archivo==""){
            return print(parse($target, $block, $append));
        }else{
              $arch=fopen($archivo,"w");
              fwrite($arch,parse($target, $block, $append));
              fclose($arch);
        }
    }
    ...
    function load_file($filename) {
       global $root;
        ...
        if(($fh = fopen("$filename", "r"))) {
            ...
            halt("load_file: No se puede abrir $filename.");
        }
        return $file_content;
    }
